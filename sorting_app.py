...
class ModernSortingApp:
    ...
    def create_ui(self):
        ...
        # Top section - File and Controls
        top_frame = ttk.Frame(main_frame, style='Modern.TFrame')
        top_frame.pack(fill=tk.X, pady=(0, 15))
        
        # File selection card
        file_card = self.create_card(top_frame, "📁 File Input")
        file_card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 10))
        ...
        sort_card = self.create_card(top_frame, "⚙️ Sorting Options")
        sort_card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        sort_inner = ttk.Frame(sort_card, style='Card.TFrame')
        sort_inner.pack(fill=tk.X, padx=15, pady=10)
        ...
        for algo in algorithms:
            rb = ttk.Radiobutton(
                ...
                command=self.update_complexity_display
            )
    ...
    def sorting_complete(self, elapsed_time):
        ...
        self.time_label.config(
            text=f"⏱️ Sorted in: {elapsed_time:.6f}s | Complexity: {complexity}"
        )
